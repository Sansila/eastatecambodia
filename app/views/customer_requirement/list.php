											...
											<select class="form-control" id="txtsearchgroup" onchange="getdata(1);">
												...
												<?php 
													...
													foreach ($customer as $cus) {
														echo '<option value="'.$cus->customerid.'">'.$cus->customer_name.'</option>';
													}
												?>
											</select>
										</th>
...
<script type="text/javascript">
	...
	$('.select-property').select2({
		...
		function getdata(page){
          	var url="<?php echo site_url('customer/getdatarequire')?>";
          	var m="<?PHP echo $m?>";
          	var p="<?PHP echo $p?>";
		...
		function deletefinding(event){
			var conf=confirm("Are you sure to delete this customer require");
			if(conf==true){
				var storeid=jQuery(event.target).attr("rel");
				var url="<?php echo site_url('customer/deleterequire')?>/"+storeid;
				$.ajax({
		            url:url,
		            type:"POST",
		            datatype:"Json",
