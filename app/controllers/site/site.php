...
class Site extends CI_Controller {
    ...
    function detail($pid)
    {
        ...
        $data['type'] = $this->site->getPropertyType();
        $data['location'] = $this->site->getPropertyLocation();
        $data['tags'] = $this->site->getPropertyTag();
    ...
    function newsdetail($newid)
    {
        ...
        $datas['profile'] = $this->site->getSiteprofile();
        $datas['menu'] = $this->site->get_menu();
        $datas['sitetype'] = "newsdetail";
        $datas['img'] = $this->site->getImageForNews($newid);
    ...
    {
        $txtsearch = $this->input->post('txtsearcharticle');
        $menuid = $_GET['type'];
        ...
        if(isset($_GET['per_page']))
            $page = $_GET['per_page'];
        $config['base_url'] = site_url('site/site/searcharticle/'.$menuid.'?type='.$menuid.'&a=link');
        $config['per_page'] = 6;
        $config['num_link'] = 3;
        $config['page_query_string'] = TRUE;
        $config['full_tag_open'] = '<ul class="pagination">';
        ...
        $query = " SELECT * FROM tblarticle WHERE is_active = 1 AND article_title LIKE '%$txtsearch%' AND is_menu = 'news' ORDER BY article_id DESC";
